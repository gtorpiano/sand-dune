...
stepsY = int(distY/cellResolution)
...
for i in range(0, stepsY):
    ...
    for j in range(0, stepsX):
        ...
        if ptOnSrf:
            row.append([ptOnSrf[0], ptOnBld])
    ptList.append(row)

#data structure has been set up: ptList

drawPts(ptList)

GRIDSIZE = 1000             #in mm, size of cell
GRIDX = len(ptList)      #no. of cells in x dir
GRIDY = len(ptList[0])         #no. of cells in y dir
...
for i in range(len(ptList)):
    ...
    for j in range(len(ptList[i])):
        ...
        if ptList[i][j][1]:
            fixMe = True
            BUILDH = int(ptList[i][j][1][0][2]/cellHeight)
